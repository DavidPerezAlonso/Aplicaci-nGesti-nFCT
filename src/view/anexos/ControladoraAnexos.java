...
import model.AnexoIII;
import model.AnexoVII;
import model.AnexoVIII;
...
public class ControladoraAnexos {
	...
	public void mostrarDatos() {
		...
		System.out.println("mostrar datos" + selectedAlumno.getNIF());
		...
		if (!nombreAnexo.getText().equals("") && nombreAnexo.getText() != null) {
			...
			if (selectedAlumno != null) {
				...
				if (anex1.isSelected()) {
					...
					AlertAnexoGenerado(nombreAnexo.getText(), anex1.getText());
				}
						...
						AlertAnexoGenerado(nombreAnexo.getText(), anex2.getText());

					} if (anex3.isSelected()){
						AnexoIII anexoIII = new AnexoIII(coledata, empdata, alumdata, cicloAnex);
						anexoIII.generarAnexoIII(nombreAnexo.getText() + "(Anexo 3)","");

						AlertAnexoGenerado(nombreAnexo.getText(), anex3.getText());


					}

					if (anex7.isSelected()){
						AnexoVII anexoVII = new AnexoVII(coledata, empdata, alumdata);
						anexoVII.generarAnexoVII(nombreAnexo.getText() + "(Anexo 7)","");

						AlertAnexoGenerado(nombreAnexo.getText(), anex7.getText());


					}

					if (anex8.isSelected()){
						AnexoVIII anexoVIII = new AnexoVIII(coledata, empdata, alumdata);
						anexoVIII.generarAnexoVIII(nombreAnexo.getText() + "(Anexo 8)","");

						AlertAnexoGenerado(nombreAnexo.getText(), anex8.getText());
	...
	// Método para unificar código de aviso infromativo de Anexo Generado
	private void AlertAnexoGenerado(String SnombreAnexo, String SnombreArchivo ){

		Alert alert = new Alert(AlertType.INFORMATION);
		alert.setTitle("Anexo generado.");
		alert.setHeaderText(null);
		alert.setContentText("¡ El anexo " + SnombreAnexo + "(" + SnombreArchivo + ") se ha generado correctamente !");
		alert.showAndWait();
